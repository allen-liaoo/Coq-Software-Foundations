...
Proof.
  intros. apply (H (fun y => x == y)). apply eq_refl.
Qed.
(** [] *)
  ...
  fun P Q R =>
    conj
      ...
      (fun (HPQPR : ((P\/Q)/\(P\/R))) =>
        ...
        | conj HPQ HPR =>
          match HPQ with
          | or_introl HP => @or_introl P (Q/\R) HP
          | or_intror HQ => 
            match HPR with
            | or_introl HP => @or_introl P (Q/\R) HP
...
Definition de_morgan_not_or : forall P Q : Prop,
    ~ (P \/ Q) -> ~P /\ ~Q :=
    fun P Q (NPOQ : P\/Q -> False) =>
      conj 
        (fun HP => 
          match (NPOQ (@or_introl P Q HP)) with end) 
        (fun HQ => 
          match (NPOQ (@or_intror P Q HQ)) with end).
(** [] *)
...
Proof.
  intros PE P H. pose proof (PE True P).
  apply H0. split.
  - intro. apply H.
  - intro. apply I.
Qed.
...
Proof.
  intros PE. unfold proof_irrelevance. intros P pf1 pf2.
  pose proof (PE P True).
  assert (P <-> True).
  { split; intros.
    - apply I.
    - apply pf1. }
  apply H in H0.
  revert pf1 pf2.  (* Crucial step to make pf1 and pf2 rewritable s*)
  rewrite H0. intros. 
