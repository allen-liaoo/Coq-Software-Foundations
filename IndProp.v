...
Proof.
    ...
    - induction n2.
        ...
        apply S_lt in H. apply IHn2 in H. apply H.
    - induction n1.
        + rewrite add_comm in H. rewrite add_0_r in H. apply H.
        + simpl in H. apply S_lt in H. apply IHn1 in H. apply H.
Qed.
(** [] *)
...
Proof.
    ...
    induction H.
    - apply leb_refl.
    - apply leb_plus_exists in IHle. destruct IHle as [x IH].
      apply plus_exists_leb. exists (S x). rewrite <- plus_n_Sm.
      f_equal. apply IH.
Qed.
...
Proof.
    ...
    - apply leb_complete in H1. apply H1.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      No, because values of m and n cant be different based on the constructors provided

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer. 
      No, because to get the condition that satisfies c4 we would have R m n o already
      *)
...
Proof.
    ...
    - intros H. induction H.
        ...
        + unfold fR in IHR. simpl in IHR. rewrite <- plus_n_Sm in IHR.
          injection IHR. intros H'. apply H'.
        + unfold fR. rewrite add_comm. apply IHR.
    - generalize dependent m. 
      generalize dependent n.
      induction o.
        + intros n m H. unfold fR in H. apply and_exercise in H.
          destruct H as [H1 H2].
          rewrite H1. rewrite H2. apply c1.
        + intros n m H. unfold fR in H. induction m.
            * simpl in H. rewrite H. apply c3. apply IHo. simpl. reflexivity.
            * apply c2. apply IHo. simpl in H. injection H. intros H1. 
            apply H1.
Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
    | sbsq_nil  (l:list nat)  : subseq nil l
    | sbsq_skip (x:nat) (l1 l2:list nat) : subseq l1 l2 -> subseq l1 (x::l2)
    | sbsq_in   (x:nat) (l1 l2:list nat) : subseq l1 l2 -> subseq (x::l1) (x::l2)
.
...
Proof.
    ...
    induction l3.
    - intros l1 l2 H. rewrite app_nil_r. apply H.
    - intros l1 l2 H. induction H.
        + apply sbsq_nil.
        + simpl. apply sbsq_skip. apply IHsubseq.
        + simpl. apply sbsq_in. apply IHsubseq.
        (* + apply sbsq_inv in IHsubseq. apply IHsubseq. *)
Qed.
...
Proof.
  (* Hint: be careful about what you are doing induction on and which
    ...
    generalize dependent l1.
    induction H0.
    - intros. inversion H. apply sbsq_nil.
    - intros. inversion H. 
        + apply sbsq_nil.
        + apply sbsq_skip. apply IHsubseq. apply H.
        + apply sbsq_skip. apply IHsubseq. rewrite H2. apply H.
    - intros. inversion H.
        + apply sbsq_nil.
        + apply sbsq_skip.  apply IHsubseq. apply H3.
        + apply sbsq_in, IHsubseq, H3.
Qed.
(** [] *)
...
Proof.
    intros. split.
    - intros. destruct H as [s E]. induction E.
        + reflexivity.
        + reflexivity.
        + simpl. apply andb_true_iff. split. 
            * apply IHE1.
            * apply IHE2.
        + simpl. apply orb_true_iff. left. apply IHE.
        + simpl. apply orb_true_iff. right. apply IHE.
        + reflexivity.
        + apply IHE2.
    - intros. induction re.
        ...
        + simpl in H. apply andb_true_iff in H. destruct H as [H1 H2].
          apply IHre1 in H1. destruct H1 as [x H1'].
          apply IHre2 in H2. destruct H2 as [y H2'].
          exists (x++y). apply MApp.
            * apply H1'.
            * apply H2'.
        + simpl in H. apply orb_true_iff in H. destruct H as [H1|H2].
            ...
            * apply IHre2 in H2. destruct H2 as [y H2'].
              exists y. apply MUnionR. apply H2'.
        + exists []. apply MStar0.
Qed.
(** [] *)
...
Proof.
    ...
    - inversion E.
      rewrite H2 in *.
      induction s1.
        + simpl. apply IHexp_match2. apply E.
        + apply IHexp_match2 in E. destruct E. destruct H1.
          exists ([x::s1]++x0). simpl. rewrite <- H1. split.
            * reflexivity.
            * intros. destruct H4.
                -- rewrite <- H4. apply H.
                -- apply H3. apply H4.
...
Lemma mstar_app : forall T (re : reg_exp T) s1 s2,
  s1 =~ Star re
  -> s2 =~ Star re
  -> (s1++s2) =~ Star re.
Proof.
    ...
    - inversion E.
    - inversion E.
    - inversion E. intros. simpl. apply H1.
    - inversion E. intros. apply IHexp_match2 with (s2:=s0) in E. 
...
Proof.
    ...
    - (* MApp *)
        ...
        apply (MApp _ _ s2 re2) in Hmatch1 as Hm1.
        {   apply add_le_cases in H.
            destruct H.
            + apply IH1 in H.
            destruct H as [s0 [s3 [s4 [E1 [E2 E3]]]]].
            exists s0, s3, (s4++s2). split.
                * rewrite E1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
                * split.
                    -- apply E2.
                    -- intros. rewrite app_assoc. rewrite app_assoc. apply (MApp ((s0 ++ napp m s3) ++ s4)).
                        ++ rewrite <- app_assoc. apply E3.
                        ++ apply Hmatch2.
            + apply IH2 in H. 
            destruct H as [s0 [s3 [s4 [E1 [E2 E3]]]]].
            exists (s1++s0), s3, s4. split.
                * rewrite E1. rewrite app_assoc. reflexivity.
                * split.
                    -- apply E2.
        ...
        (* assert (L: pumping_constant re1 <= length s1 \/ pumping_constant re1 > length s1).
        { rewrite or_comm. apply lt_ge_cases. }
        destruct L. admit. admit. *)
    - (* MUnionL *)
        intros H. simpl in H. apply lt_n_m in H. apply IH in H. 
        destruct H as [s2 [s3 [s4 [E1 [E2 E3]]]]].
        exists s2, s3, s4. split.
        + apply E1.
        + split. 
            * apply E2.
            * intros. apply MUnionL. apply E3.
    - (* MUnionR *)
        intros H. simpl in H. rewrite add_comm in H. apply lt_n_m in H. apply IH in H. 
        destruct H as [s1 [s3 [s4 [E1 [E2 E3]]]]].
        exists s1, s3, s4. split.
        + apply E1.
        + split.
            * apply E2.
            * intros. apply MUnionR. apply E3.
    - (* MStar0 *) intros H. simpl in H. inversion H. apply pumping_constant_0_false in H1. inversion H1.
    - (* MStarApp *) intros H. simpl in H.
      exists [], (s1++s2), []. split.
        + simpl. rewrite app_nil_r. reflexivity.
        + split. 
            * destruct (s1++s2).
                -- simpl in H. apply PeanoNat.Nat.le_0_r in H. 
...
Lemma le_two_sums : forall a b c d,
    a <= b -> c <= d -> a + c <= b + d.
Proof.
    intros a b c d H.
    ...
    induction H. 
    - intros. apply plus_le_compat_l. apply H.
    - intros. apply IHle in H0. simpl. apply le_S. apply H0.
Qed.

Lemma napp_rep_star : forall T m s (re : reg_exp T),
...
Proof.
  ...
  induction Hmatch
    ...
    - intros. inversion H. inversion H1. 
    - (* MApp *)
      intros. destruct (lt_ge_cases (pumping_constant re1) (length s1)) as [H1|H2].
        + unfold lt in H1. apply (S_le (pumping_constant re1) _) in H1.
          apply IH1 in H1. destruct H1 as [s0 [s3 [s4 [E1 [E2 [E3 E4]]]]]].
          exists s0, s3, (s4++s2).
          split; [|split; [|split]].
            * rewrite E1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
            * apply E2.
            * simpl. apply le_plus_trans. apply E3.
            * intros. rewrite app_assoc. rewrite app_assoc. apply (MApp ((s0 ++ napp m s3) ++ s4)).
                -- rewrite <- app_assoc. apply E4.
                -- apply Hmatch2.
        +  simpl in H. rewrite app_length in H. apply add_le_cases in H.
           destruct H as [H1|H3].
            * apply IH1 in H1. destruct H1 as [s0 [s3 [s4 [E1 [E2 [E3 E4]]]]]].
              exists s0, s3, (s4++s2). split; [|split; [|split]].
                -- rewrite E1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
                -- apply E2.
                -- simpl. apply le_plus_trans. apply E3.
                -- intros. rewrite app_assoc. rewrite app_assoc. apply (MApp ((s0 ++ napp m s3) ++ s4)).
                    ++ rewrite <- app_assoc. apply E4.
                    ++ apply Hmatch2.
            * apply IH2 in H3. destruct H3 as [s0 [s3 [s4 [E1 [E2 [E3 E4]]]]]].
              exists (s1++s0), s3, s4. split.
                -- rewrite E1. rewrite <- app_assoc. reflexivity.
                -- split.
                    ** apply E2.
                    ** split.
                        ++ simpl. rewrite app_length. rewrite <- add_assoc. apply le_two_sums. 
                            { apply H2. }
                            { apply E3. }
                        ++ intros. rewrite <- app_assoc. apply MApp. 
                            { apply Hmatch1. }
    ...
    - (* MUnionR *)
        ...
        + split.
            * apply E2.
            * split.
                -- simpl. replace (pumping_constant re1 + pumping_constant re2) with (pumping_constant re2 + pumping_constant re1).
                    apply le_plus_trans. apply E3.
                    { rewrite add_comm. reflexivity. }
                -- intros. apply MUnionR. apply E3.
    - (* MStar0 *) intros H. simpl in H. inversion H. apply pumping_constant_0_false in H1. inversion H1.
    - (* MStarApp *)
        intros H. simpl in H.
        destruct (lt_ge_cases (pumping_constant re) (length s1)) as [H1|H2].
        + simpl. unfold lt in H1. apply (S_le (pumping_constant re) _) in H1.
          apply IH1 in H1. destruct H1 as [s0 [s3 [s4 [E1 [E2 [E3 E4]]]]]].
          exists s0, s3, (s4++s2). repeat split.
            * rewrite E1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
            * apply E2.
            * apply E3.
            * intros. rewrite app_assoc. rewrite app_assoc. apply (MStarApp ((s0++napp m s3)++s4)).
              -- rewrite <- app_assoc. apply E4.
              -- apply Hmatch2.
        + destruct s1.
            ...
            * exists [], (x::s1), s2. repeat split.
              ...
              -- apply H2.
              -- intros. rewrite app_assoc. change ([ ] ++ napp m (x :: s1)) with (napp m (x :: s1)).
                 apply star_app. 
                  ** apply napp_rep_star. apply Hmatch1.
                  ** apply Hmatch2.
Qed.
...
Proof.
  ...
  - simpl. unfold not in IHl'. unfold not. intros. destruct H as [H1|H2].
    ...
    + simpl in Hcount. destruct (eqbP n m) as [H | H].
      * discriminate Hcount.
      * simpl in Hcount. apply IHl' in Hcount. contradiction Hcount. apply H2.
Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
    | nstter_nil : nostutter nil
    | nstter_cons (x:X) : nostutter (x::nil)
    | nstter_cons2 (x y:X) (l:list X) (E: x <> y) (H:nostutter (y::l)) : nostutter (x::y::l)
.
...
Inductive merge {X:Type} : list X -> list X -> list X -> Prop :=
    ...
    | mergeL (x:X) (l1 l2 l:list X) : merge l1 l2 l -> merge (x::l1) l2 (x::l)
    | mergeR (y:X) (l1 l2 l:list X) : merge l1 l2 l -> merge l1 (y::l2) (y::l)
.
...
Theorem filter_challenge_2 : forall (test: nat->bool) (sl l:list nat),
  subseq sl l ->
...
Proof.
  ...
  - intros. apply IHsubseq in H0. simpl. destruct (test x).
    + simpl. apply le_S in H0. apply H0.
    + apply H0.
  - intros. simpl in H0. destruct H0 as [H0' H0]. 
    apply IHsubseq in H0. simpl. destruct (test x).
    + simpl. apply n_le_m__Sn_le_Sm. apply H0.
    + discriminate H0'.
Qed.
(* [] *)
...
Inductive pal {X:Type} : list X -> Prop :=
  | pal_nil : pal nil
  | pal_one (x:X) : pal [x]
  | pal_two (x:X) (p:list X) (H: pal p) : pal ((x::p)++[x]).
...
Proof.
  intros. induction l.
  - simpl. apply pal_nil.
  - simpl. replace (x::(l ++ rev l ++ [x])) with (x::(l ++ rev l) ++ [x]). apply pal_two, IHl.
...
Proof.
  ...
  - reflexivity.
  - reflexivity.
  - change (rev ((x :: p) ++ [x])) with (rev (p ++ [x]) ++ [x]).
    rewrite rev_app_distr. simpl. rewrite <- IHpal. reflexivity.
Qed.
(** [] *)
...
Lemma last_el_list_ident : forall {X:Type} (a b:X) (l1 l2:list X),
    l1++[a] = l2++[b] -> l1 = l2 /\ a = b.
Proof.
  intros X a b l1. generalize dependent b. generalize dependent a.
  induction l1.
  ...
  - intros. destruct l2.
    + simpl in H. inversion H. destruct l1.
      * discriminate H2.
      * discriminate H2.
    + inversion H. apply IHl1 in H2. destruct H2 as [H2 H3]. split.
...
Proof.
  ...
  - right. destruct IHl.
    ...
    + destruct H as [x0 [f0 H]].
      exists x0, (x::f0).
      rewrite H. reflexivity.
Qed.

Lemma palindrom_converse_by_length: forall {X:Type} (n:nat) (l:list X),
    length l <= n -> l = rev l -> pal l.
Proof.
  induction n.
  - intros. destruct l.
    + apply pal_nil.
    + simpl in H. apply PeanoNat.Nat.nle_succ_0 in H. destruct H.
  - intros. destruct n.
    + destruct l.
      * apply pal_nil.
      * simpl in H. apply Sn_le_Sm__n_le_m in H. apply PeanoNat.Nat.le_0_r in H.
        assert (forall {X:Type} (l:list X), length l = 0 -> l = []) as length_0_empty.
        {
          intros. destruct l0.
          - reflexivity.
          - simpl in H1. discriminate H1.
        }
        apply length_0_empty in H. rewrite H. apply pal_one.
    + destruct l.
      * apply pal_nil.
      * destruct (list_split l).
        -- rewrite H1. apply pal_one.
        -- destruct H1 as [x0 [f0 H1]]. rewrite H1.
           simpl in H0. rewrite H1 in H0.
           apply (last_el_list_ident x0 x (x::f0) (rev (f0++[x0]))) in H0.
           destruct H0 as [H0 H2]. rewrite H2 in *.
           apply pal_two. apply IHn.
           ++ simpl in H. apply Sn_le_Sm__n_le_m in H.
              rewrite H1 in H. rewrite app_length in H.
              apply plus_le in H. destruct H as [H _]. apply H.
           ++ rewrite rev_app_distr in H0. simpl in H0. inversion H0.
              rewrite rev_involutive. symmetry. apply H4.
Qed.

Theorem palindrome_converse: forall {X: Type} (l: list X),
    l = rev l -> pal l.
Proof.
  Check palindrom_converse_by_length.
  intros X l.
  apply (palindrom_converse_by_length (length l)). reflexivity.
Qed.
(** [] *)
...
Definition disjoint {X:Type} (l1 l2:list X): Prop :=
  ...
  (* | dis_nil_fst  (l2:list X)               : disjoint nil l2
  | dis_nil_snd  (l1:list X)               : disjoint l1 nil
  | dis_hd_fst   (h1:X) (l1 l2:list X)     : ~(In h1 l2) -> disjoint l1 l2 -> disjoint (h1::l1) l2
  | dis_hd_snd   (h2:X) (l1 l2:list X)     : ~(In h2 l1) -> disjoint l1 l2 -> disjoint l1 (h2::l2) *)
.
(* FILL IN HERE *)
...
Inductive nodup {X:Type} : list X -> Prop :=
  | nodup_nil : nodup nil
  | nodup_hd (x:X) (l:list X) : ~(In x l) -> nodup l -> nodup (x::l)
.
(* FILL IN HERE *)
...
Proof.
  ...
  - discriminate H.
Qed.
(* This lemma is used in regex proofs at the end of the document *)

Lemma in_app : forall {X:Type} (x:X) (l:list X),
  In x l -> exists l1 l2, l = l1 ++ [x] ++ l2.
Admitted.

Theorem nodup_no_two_in : forall {X:Type} (x:X) (l1 l2:list X),
  In x l1 -> In x l2 -> 
  ~(nodup (l1++l2)).
Proof.
  intros X x l1.
  ...
  - intros. induction l2.
    (* + 
    + unfold not. intro. inversion H3. simpl in H1. simpl in H2. *)
Admitted.
...
Proof.
  intros X l1 l2 H.
  remember (l1++l2) as l12.
  induction H.
  - symmetry in Heql12. apply nil_app_nil in Heql12. destruct Heql12 as [H1 H2].
    rewrite H1. rewrite H2. unfold disjoint. intro. destruct H as [x [H3 H4]].
    inversion H3.
  - clear IHnodup. unfold not in H. unfold disjoint. intro. destruct H1 as [y [H3 H4]].
    inversion H0.
    + rewrite <- H1 in Heql12. destruct l1.
      * inversion H3.
      * inversion Heql12. symmetry in H6. apply nil_app_nil in H6. destruct H6 as [H6 H7].
        rewrite H7 in H4. inversion H4.
    + pose proof (nodup_no_two_in y _ _ H3 H4) as P.
      pose proof (nodup_hd x l H H0) as P1. rewrite Heql12 in P1. contradiction.
Admitted.
(* FILL IN HERE *)
...
Proof.
  ...
  - simpl in H. destruct H as [H|H].
    + exists [], l. rewrite H. auto.
    + apply IHl in H. destruct H as [l1 [l2 H]].
      exists (x0::l1), l2. rewrite H. auto.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | rp_skip (x:X) (l:list X) : repeats l -> repeats (x::l)
  | rp_in (x:X) (l:list X) : In x l -> repeats (x::l)
.
...
Proof.
  ...
  - intros. pose proof (EM (In x l1')) as E. destruct E as [E|E].
    + apply rp_in in E. apply E.
    + specialize (H x). assert (In x (x::l1')).
      { simpl. left. reflexivity. }
      apply H in H1. apply in_split in H1. 
      destruct H1 as [l2' [l2'' H1]].
      specialize (IHl1' l2').
      (* can prove the rest w lemmas and IH *)
Admitted.
(** [] *)

(* Proving pigeonhole w/o LEM:
We collect elements in l2 seen in l1 into some list u *)
Theorem pigeonhole_principle_no_lem:
  forall (X:Type) (l1 l2:list X) (u:list X),
  (forall x, In x l1 -> In x (u++l2)) ->
  length l2 < length l1 ->
  repeats (u++l1).
Proof. Admitted.
...
Proof.
  intros. split.
  - intros H. 
    apply app_exists in H. destruct H as [s0 [s1 H]]. 
    destruct s0.
    + simpl in H. left. destruct H as [H [H1 H2]]. split.
      * apply H1. 
      * rewrite H. apply H2.
    + right. destruct H as [H [H1 H2]]. inversion H. 
      rewrite H3 in *. exists s0, s1. repeat split.
      * apply H1.
      * apply H2.
  - intros H. destruct H as [[H1 H2]|H].
    + change (a::s) with ([]++a::s). apply MApp.
      * apply H1.
      * apply H2.
    + destruct H as [s0 [s1 [H1 [H2 H3]]]]. rewrite H1. 
...
Proof.
  intros a s re. split.
  - intros H. remember (Star re) as s' eqn:E.
    remember (a::s) as a's eqn:E1.
    induction H.
    + discriminate E.
    + discriminate E.
    + discriminate E.
    ...
    + discriminate E.
    + discriminate E1.
    + inversion E. rewrite H2 in *. destruct s1.
      * simpl in E1. apply (IHexp_match2 E) in E1. apply E1.
      * inversion E1. rewrite H3 in *. exists s1, s2. repeat split.
        -- apply H.
        -- apply H0.
  - intros. destruct H as [s0 [s1 [H1 [H2 H3]]]]. 
    rewrite H1. change (a::s0++s1) with ((a::s0)++s1). 
    apply MStarApp.
      + apply H2.
      + apply H3.
Qed.
...
Proof.
    ...
    - simpl. destruct (match_eps re1).
      + destruct (match_eps re2).
        * simpl. apply ReflectT. change [] with ((@nil ascii)++[]). apply MApp. 
          { inversion IHre1. apply H. }
          { inversion IHre2. apply H. }
        * simpl. apply ReflectF. unfold not. intro A.
          inversion IHre2. unfold not in H. apply H.
          inversion A. 
          apply (nil_app_nil s1 s2) in H1. destruct H1 as [H1 H1']. 
          rewrite H1. rewrite H1' in *. apply H4.
      + simpl. apply ReflectF. unfold not. intro A.
        inversion IHre1. inversion A.
        apply (nil_app_nil s1 s2) in H1. destruct H1 as [H1 H1']. 
        rewrite H1 in H3. contradiction.
    - simpl. destruct (match_eps re1).
      + simpl. apply ReflectT. apply MUnionL.
        inversion IHre1. apply H.
      + destruct (match_eps re2).
        * simpl. apply ReflectT. apply MUnionR.
          inversion IHre2. apply H.
        * simpl. apply ReflectF. inversion IHre1. inversion IHre2.
          unfold not. intro U. inversion U. 
          -- unfold not in H. apply (H H3).
          -- unfold not in H0. apply (H0 H3).
...
Fixpoint derive (a : ascii) (re : reg_exp ascii) : reg_exp ascii :=
  match re with
  | EmptySet => EmptySet
  | EmptyStr => EmptySet  (* expects no more characters but recieves one *)
  | Char x => if Ascii.eqb a x then EmptyStr else EmptySet
  | App r1 r2 => 
      Union 
        (if match_eps r1 then derive a r2 else EmptySet) (* EmptySet here is fine as the other option incorporates it*)
        (App (derive a r1) r2)
    (* if match_eps r1 then derive a r2
    else (let rst := derive a r1 in
      if match_eps rst then r2
      else App rst r2) *)
...
Lemma nil_MApp : forall {T:Type} (r1 r2:reg_exp T),
  [] =~ App r1 r2 -> [] =~ r1 /\ [] =~ r2.
Proof.
  intros. change [] with (@nil T++[]) in H. inversion H. apply nil_app_nil in H1. 
    destruct H1 as [H1 H1'].
    rewrite H1 in *. rewrite H1' in *. simpl. split.
    - apply H3.
    - apply H4.
Qed.
...
Proof.
  ...
  unfold is_der. intro s. split.
  - intro H. 
    generalize dependent s. induction re.
    + intros. inversion H.
    + intros. inversion H.
    + intros. simpl. destruct (eqb_spec a t).
      * rewrite e in H. apply char_eps_suffix in H. rewrite H. apply MEmpty.
      * apply (char_nomatch_char _ _ _ n) in H. contradiction.
    + simpl. intros. apply app_ne in H.
      apply union_disj.
      destruct H as [[H H1]|H2].
      * destruct (match_eps_refl re1).
        -- left. apply IHre2. apply H1.
        -- contradiction.
      * right. destruct H2 as [s0 [s1 [H [H1 H2]]]].
        apply IHre1 in H1. rewrite H. apply MApp.
        -- apply H1.
        -- apply H2.
    + intros. simpl. apply union_disj in H. destruct H as [H1|H2].
      * apply IHre1 in H1. apply MUnionL. apply H1.
      * apply IHre2 in H2. apply MUnionR. apply H2.
    + intros. simpl. apply star_ne in H. destruct H as [s0 [s1 [H [H1 H2]]]].
      rewrite H. apply MApp. 
      * apply IHre. apply H1.
      * apply H2.

  (* remember (a::s) as re' eqn:E. induction H.
    + discriminate E.
    + inversion E. simpl. destruct (eqb_spec a a).
      * apply MEmpty.
      * contradiction.
    + simpl. apply union_disj. destruct (match_eps_refl re1). admit. (*destruct (match_eps re1).*)
    + simpl. apply MUnionL. apply IHexp_match. apply E.
    + simpl. apply MUnionR. apply IHexp_match. apply E.
    + discriminate E.
    + simpl. apply app_exists.*)
  - intros H.
    ...
    + intros. simpl in H. inversion H.
    + intros. simpl in H. destruct (eqb_spec a t).
      * rewrite e. apply char_eps_suffix. apply empty_matches_eps. apply H.
      * apply char_nomatch_char. { apply n. } { inversion H. }
    + intros. simpl in H. apply union_disj in H. destruct H as [H1|H2]. 
      * destruct (match_eps_refl (re1)). apply IHre2 in H1.
        ...
        -- inversion H1.
      * destruct s.
        -- apply nil_MApp in H2. destruct H2 as [H2 H2']. apply (IHre1 [] a) in H2. apply (MApp [a]).
          ++ apply H2.
          ++ apply H2'.
        -- apply app_ne in H2. destruct H2 as [[H H1]|[s0 [s1 H]]].
          ++ apply IHre1 with (a:=a) in H. apply (MApp [a] re1 (x::s) re2 H H1).
          ++ destruct H as [H [H1 H2]].
             apply IHre1 with (a:=a) in H1. rewrite H. apply (MApp _ re1 _ re2 H1 H2).
    + intros. simpl in H. apply union_disj in H. apply union_disj. destruct H as [H|H].
      * left. apply IHre1, H.
      * right. apply IHre2, H.
    + intros. simpl in H. apply star_ne. apply app_exists in H. destruct H as [s0 [s1 [H [H1 H2]]]].
...
Proof.
  ...
  intros. generalize dependent re.
  induction s. 
  ...
  - intros. simpl. 
    ...
    (* Got help from internet; this is the crucial step, noticing that IHs supplied with (derive x re) can destruct expression
       in the goal to true/false *)
    + apply ReflectT. destruct (derive_corr x re s). apply H1 in H. apply H. 
    + apply ReflectF. intro. destruct (derive_corr x re s). 
      unfold not in H. apply H1 in H0. apply H in H0. destruct H0.
Qed.
(** [] *)
