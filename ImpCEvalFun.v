...
Example peven_test_even :
  test_ceval (X !-> 6) peven
  = Some (0, 0, 0).
Proof. reflexivity. Qed.

Example peven_test_odd :
...
Proof.
  ...
  - (* i = S i' *)
    ...
    destruct c;
      ...
      + (* ; *)
        destruct (ceval_step st c1 i') eqn:Heqr1C.
        * (* Evaluation of r1 terminates normally *)
          apply E_Seq with s.
            apply IHi'. rewrite Heqr1C. reflexivity.
...
Base case: If i = 0, then "None = Some st'", which is false.
Inductive case: If i = S i',
The inductive hypothesis is "forall c st st', ceval_step st c i' = Some st' -> st =[ c ]=> st'"
Assuming "ceval_step st c (S i') = Some st'", we want to prove that c takes st to st'.
  - skip: ceval_step evaluates the arguments to Some st, where Some st = Some st', 
    so st = st'. We then prove that skip takes st to st, which is correct.
  - assignment: after ceval_step evaluates, we get "st' = (l !-> aeval st a1 ; st)",
    which is true of assignment.
  - sequence: Given that the command is "s1; s2":
    If ceval_step with st and i' returns None, then we have None = Some st', which is false.
    If it is Some st'', then we get that ceval_step st'' c i' = Some st'. We can use these two facts
    in proving the goal.
  - if: Given that the command is "if b then c1 else c2 end"
    If b evaluates to true, then we have "ceval_step st c1 i' = Some st'", from that it is obvious that the goal is provable.
    Simialrly when b evaluates to false.
  - while: Given that the command is "while b do c end"
    If b evaluates to false, then we have that the whole thing gets to Some st, where st = st', then the goal is trivially true.
    If b evaluates to true, then we consider that if one execution of c terminates in i' steps.
      If it terminates, then we have that the while command again terminates in i' steps, yeilding st'. The goal is then provable.
      If it doesn't terminate, it violates our assumption.
 *)
...
Proof.
  intros c st st' Hce.
  induction Hce.
  1: exists 1. reflexivity.
  (* Assignment and While-False: 1 step *)
  1,5: exists 1; simpl; rewrite H; reflexivity.
  (* If Then (True and False case) *)
  2-3: destruct IHHce as [i IH]; exists (S i);
        simpl; rewrite H; rewrite IH; reflexivity.
  (* Sequence and While-True *)
  all: destruct IHHce1 as [i1 IH1]; destruct IHHce2 as [i2 IH2];
    exists ((S i1) + (S i2)); simpl;
    try (rewrite H);
    apply (ceval_step_more i1 (i1+S i2) _ _ _ (le_add_r i1 (S i2))) in IH1;
    rewrite IH1;
    apply (ceval_step_more i2 (i1+S i2) _ _ _) in IH2.
    2,4: lia. 
  all: rewrite IH2;  reflexivity.
Qed.
(** [] *)
