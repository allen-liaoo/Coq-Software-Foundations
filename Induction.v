...
Proof.
    induction n as [|n IH].
    - simpl. reflexivity.
    - rewrite IH. simpl. rewrite negation_fn_applied_twice. reflexivity.
        + reflexivity. (* to dispense w the assumption of negation_fn_applied_twice *)
Qed.
(** [] *)
...
Theorem add_comm : forall n m : nat,
    ...
    Proof:
    We induct on n.
    Suppose n = 0. Then we have 0 + m = m + 0, which follows from the definition of +.
    Suppose n = n' + 1 for some natural number n', where
    n' + m = m + n' (IH).
    So, we have
    (n' + 1) + m = m + (n' + 1)
    Following the definition of +, we have
    (n' + m) + 1 = m + (n' + 1)
    Since we have (x + y) + 1 = x + (y + 1) for any x and y,
    (n' + m) + 1 = (m + n') + 1.
    By our inductive hypothesis, we have proved that n + m = m + n.
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    We induct on n.
    Suppose n = 0.
    Then we have 0 =? 0, which is true, following the definition of =?.

    Suppose n = n' + 1 for some n' s.t. (n' =? n') = true (IH).
    Following the definition of =?, we have that (n'+1) =? (n'+1) if n' =? n', 
    which is immediate from the inductive hypothesis.
*)
...
Theorem mul_distr : forall k n : nat,
    n * S k = n + n * k.
Proof.
    intros.
    induction n as [|n' IH].
    - simpl. reflexivity.
    - simpl. rewrite IH. rewrite add_assoc. rewrite add_assoc. 
    assert (H2:k+n' = n' + k).
    { rewrite add_comm. reflexivity. }
...
Proof.
    ...
    - simpl. rewrite mul_distr. rewrite IH. reflexivity.
Qed.
(** [] *)
...
Proof.
    ...
    - rewrite mul_distr. rewrite mul_distr. rewrite mul_distr.
    rewrite <- add_assoc. 
    assert (T: forall x, m + x = x + m).
    { intros. rewrite add_comm. reflexivity. }
    rewrite T. rewrite T. rewrite IH. 
...
Proof.
    ...
    rewrite IH1. 
    assert (R: 1 + bin_to_nat b1 = S (bin_to_nat b1)).
    { reflexivity. }
    rewrite R. rewrite <- plus_n_Sm. reflexivity.
Qed.
...
(* There are bins that are equivalent. 
    For example, Z is equivalent to B0 B0 B0 Z.
    B1 Z is equivalent to B1 B0 Z. *)
...
Fixpoint peal_b0_rep (b : bin) : bin :=
    match b with
    | B0 Z => Z
    | B0 b' => peal_b0_rep b'
    | _ => Z
...
Example normalize_1:
    normalize (B1 (B0 (B0 Z))) = B1 Z.
Proof. simpl. reflexivity. Qed.

Example normalize_101:
    normalize (B1 (B0 (B1 (B0 (B0 (B0 Z)))))) = B1 (B0 (B1 Z)).
Proof. simpl. reflexivity. Qed.

(* Lemmas for the last proof: *)

Lemma double_bin_unfold : forall b : bin,
    double_bin b = match b with
    | Z => Z
    | B0 c => B0 (B0 c)
    | B1 c => B0 (B1 c)
    end.
...
Lemma nat_to_double_bin : forall n : nat,
    nat_to_bin (n + n) = double_bin (nat_to_bin n).
Proof.
    induction n as [|n IH].
    - simpl. reflexivity.
    - simpl. rewrite <- plus_n_Sm. simpl. 
    rewrite IH. rewrite double_incr_bin. reflexivity.
Qed.
...
Proof.
    induction b as [|b0 IH0|b1 IH1].
    - simpl. reflexivity.
    - replace (bin_to_nat (B0 b0)) with (bin_to_nat b0 + bin_to_nat b0).
    rewrite nat_to_double_bin. simpl. 
    rewrite <- IH0. rewrite double_bin_unfold. reflexivity.
        { simpl. rewrite add_0_r. reflexivity. }
    - simpl. rewrite add_0_r. rewrite nat_to_double_bin.
    rewrite IH1. rewrite incr_double_bin_b1. reflexivity.
Qed.
