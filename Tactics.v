...
Proof.
    ...
    - simpl. destruct (split t) as [tx ty].
...
Proof.
    ...
    destruct b eqn:B.
    - destruct (f b) eqn:FB.
        + rewrite B in FB. rewrite FB. rewrite FB. rewrite FB. reflexivity.
        + rewrite B in FB. rewrite FB. destruct (f false) eqn:FF.
            * rewrite FB. reflexivity.
            * rewrite FF. reflexivity.
    - destruct (f b) eqn:FB.
        + rewrite B in FB. rewrite FB.  destruct (f true) eqn: FT.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   (* 
    We induct on n.
    Suppose n = 0. Then either m = 0 or m = S m'.
        If m = 0, then (0 =? 0) = (0 =? 0).
        If m = S m', then (0 =? S m') = false = (S m' = 0).
    Suppose n = S n'. We prove by the same cases:
        If m = 0, then (S n' =? 0) = false  = (S m' = 0).
        If m = S m', then (S n' =? S m') = (S m' =? S n') if (n' =? m') = (m' =? n'), which we have by the IH.
    [] *)
...
Proof.
    ...
    - intros l2 H. destruct l2.
        + discriminate H.
        + injection H as H. apply IH in H.
...
Proof.
    ...
    - simpl in H. destruct (test h) eqn:T.
        + injection H as H1 H2. rewrite <- H1. apply T.
        + apply IH. apply H.
Qed.
(** [] *)
...
Proof. 
    ...
    - simpl. destruct (test h) eqn:T.
      + unfold existsb'. simpl. rewrite T. reflexivity.
      + unfold existsb'. simpl. rewrite T. simpl. rewrite IH. reflexivity.
Qed.
