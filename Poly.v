...
(*
- type of combine is: forall X Y: Type, list X => list Y => list (X*Y)
- same thing
- [(1,false);(2,false)] *)
...
Definition filter_even_gt7 (l : list nat) : list nat :=
    filter (fun x => (even x) && (negb (leb x 7))) l.
...
(* Converting a list of non-strings into string
    [] *)
...
(* 
Suppose length l = n.
We induction on the length of l.
- If length l = 0, then then @nth_error X l n = None.
- For the IH, suppose that @nth_error X l' k = None.
Since k+1 is not 0, @nth_error X l (k+1) = nth_error l' k, which we know is None from the IH. *)
