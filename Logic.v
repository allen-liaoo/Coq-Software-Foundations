...
(* 
Take some arbitrary P, suppose P.
Notice that ~~P is (P → False) → False. 
So we assume P → False. 
To prove "False" to get ~~P, we must prove P, which follows from our assumption that P.
Qed.
*)
...
(* 
Take some arbitrary P. 
Notice that ~(P /\ ~P) is equivalent to (P /\ ~P) -> False.
Assume (P /\ ~P), which is two assumptions, P and P -> False.
To prove "False", we need to prove P, but we already assumed P.
Qed.
 *)
...
Proof.
    ...
    unfold not in H.
    split.
    - unfold not. intros PH. apply H. left. apply PH.
    - unfold not. intros QH. apply H. right. apply QH.
Qed.
(** [] *)
...
Proof.
    ...
    intros HIFF1 HIFF2.
    split.
    - intros HP. apply HIFF2. apply HIFF1. apply HP.
    - intros HR. apply HIFF1. apply HIFF2. apply HR.
Qed.
(** [] *)
Search (_ \/ _) in Logic.
...
Proof.
    ...
    - intros H. split.
        ...
        + destruct H as [HP | HQR].
            * left. apply HP.
            * right. apply HQR.
    - intros H. destruct H as [HPQ HPR]. destruct HPQ as [HP|HQ].
        + left. apply HP.
...
Proof.
    intros X P Q.
    split.
    - intros [x E]. destruct E as [H1|H2].
        + left. exists x. apply H1.
        + right. exists x. apply H2.
    - intros H. destruct H as [H1|H2].
        + destruct H1 as [x H1]. exists x. left. apply H1.
        + destruct H2 as [x H2]. exists x. right. apply H2.
Qed.
(** [] *)
...
Proof.
    ...
    induction m as [|m IH].
    - intros n H. exists 0. destruct n. { reflexivity. } { discriminate H. }
    - intros n H. destruct (n <=? m) eqn:D.
        + apply IH with (n:=n) in D. destruct D as [x D].
          exists (x+1). rewrite (add_comm x 1). simpl.
          rewrite <- plus_n_Sm. f_equal. apply D.
        + exists 0. rewrite add_comm. simpl.
          destruct (n =? S m) eqn:L.
            * symmetry. apply eqb_true. apply L.
            (* * symmetry. apply eqb_true. apply (leq_eqb n m) in H.  *)
Admitted.
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    + simpl. intros [].
    + simpl. intros C. destruct C as [C1|C2].
        * exists x. split. { apply C1. } { left. reflexivity. }
        * apply IHl' in C2. destruct C2 as [x1 [C2' C2'']]. 
        exists x1. split. { apply C2'. } { right. apply C2''. }
  - induction l as [|xl l' IH].
    + intros [x H]. destruct H as [H1 H2]. simpl in H2. destruct H2.
...
Proof.
  ...
  - intros l'' a. simpl. split.
    + intro H. destruct H as [H1|H2]. 
        * left. left. apply H1.
        * apply IH in H2. apply or_assoc. right. apply H2.
    + intro H. destruct H as [H1|H2].
...
Proof.
    ...
    - simpl. split. 
      { intro H. apply I. }
      { intros H x. apply False_ind. }
    - simpl. split.
        * intros H. split.
            { apply H. left. reflexivity. }
            { apply IH. intros x H'. apply H. right. apply H'. }
        * intros H x H'. destruct H' as [H1'|H2'].
            + rewrite <- H1'. apply H.
            + destruct IH as [IH1 IH2]. apply IH2. 
...
Proof.
    ...
    unfold combine_odd_even. destruct (odd n) eqn:O. 
...
Proof.
    ...
    - rewrite even_S. destruct (even n'). 
        + simpl. destruct IH as [k IH]. exists k. rewrite IH. reflexivity.
        + simpl. destruct IH as [k IH]. exists (S k). rewrite IH. reflexivity.
Qed.
(** [] *)
...
Proof.
    ...
    - intros H. unfold not. intros H1. 
      apply not_true_iff_false in H. unfold not in H.
      apply eqb_eq in H1. apply H in H1. apply H1.
    - intros H. unfold not in H. 
      apply not_true_iff_false. unfold not.
      intros H1. apply eqb_eq in H1. apply H in H1. apply H1.
Qed.
(** [] *)
...
Proof.
    intros A eq H l1 l2. split.
    - generalize dependent l2. induction l1 as [|h t IH].
        + intros l2 H1. simpl in H1. destruct l2.
            * reflexivity.
            * discriminate H1.
        + intros l2 H1. destruct l2 as [|h2 t2].
            * discriminate H1.
            * simpl in H1. destruct (eq h h2) eqn:T.
                ++ apply IH in H1. apply H in T.
                f_equal. { apply T. } { apply H1. }
                ++ discriminate H1.
    - generalize dependent l2. induction l1 as [|h t IH].
        + intros l2 H1. rewrite <- H1. reflexivity. 
        + intros l2 H1. rewrite <- H1. simpl. destruct (eq h h) eqn:E.
            * apply IH. reflexivity.
            * rewrite <- E. apply H. reflexivity.
Qed.
...
Proof.
    ...
    - induction l as [|h t IH].
        + reflexivity.
        + intros H. simpl in H. simpl. destruct (test h) eqn:E.
            * split. 
              { reflexivity. } 
              { apply IH. apply H. }
            * discriminate H.
    - induction l as [|h t IH].
        + reflexivity.
        + intros H. simpl. destruct (test h) eqn:E.
            * apply IH. apply H. 
            * simpl in H. destruct H as [H1 H2]. rewrite H1 in E. discriminate E.
Qed.
...
Proof.
    ...
    - reflexivity.
    - intros l2. simpl. rewrite IH. 
      assert (H: rev_append t [h] = rev_append t [] ++ [h]).
...
Proof.
    ...
    - reflexivity.
    - unfold tr_rev in IH. unfold tr_rev. simpl. rewrite <- IH. 
      rewrite rev_append_correct. reflexivity.
Qed.
(** [] *)
...
Proof.
    intros EM X P H x. 
    specialize (EM (P x)).
    destruct EM as [E1|E2].
    - apply E1.
    - unfold not in H. unfold not in E2. 
      assert (E: (P x -> False) -> (exists x, P x -> False)).
      { intros H1. exists x. apply E2. }
      apply E in E2. apply H in E2. exfalso. apply E2.
Qed.
(** [] *)
...
Proof.
    ...
    intros EM P Q H.
    specialize (EM P) as EM'.
    destruct EM' as [H1|H2].
    - apply H1.
    - unfold not in H2. apply H. 
      intros H3. apply H2 in H3. exfalso. apply H3.
Qed.

Theorem peirce_to_dne : peirce -> double_negation_elimination.
Proof.
    unfold peirce. unfold double_negation_elimination.
    intros H P. unfold not. 
    intros H1. apply H  with (Q:=False). 
    intros H3. apply H1 in H3. 
    exfalso. apply H3.
Qed.

Theorem dne_to_dmnn : double_negation_elimination -> de_morgan_not_and_not.
    unfold double_negation_elimination. unfold de_morgan_not_and_not.
    intros DNE P Q. 
    intros DM. unfold not in DM. 
    apply DNE. unfold not. intros OR.
    apply DM. split.
    - intros PH. apply OR. left. apply PH.
    - intros QH. apply OR. right. apply QH.
Qed.

Theorem dmm_to_ito : de_morgan_not_and_not -> implies_to_or.
Proof. 
    ...
    apply H1. intros PH.
    apply IM in PH. apply H2 in PH. apply PH.
Qed.    
